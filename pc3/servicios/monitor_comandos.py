...
class MonitorComandos(threading.Thread):
    ...
    def _imprimir_estados(self) -> None:
        ...
        # Agrupar por interseccion base (ej. INT_A1)
        agrupados = {}
        for r in rows:
            ...
            if base not in agrupados:
                agrupados[base] = {"NS": {"estado": "SIN DATOS", "sensor": "--"}, 
                                   "EO": {"estado": "SIN DATOS", "sensor": "--"}}
                                   
            if direccion in agrupados[base]:
                agrupados[base][direccion] = {"estado": estado, "sensor": sensor or "N/A"}
            else:
                agrupados[base][direccion] = {"estado": estado, "sensor": sensor or "N/A"}
        ...
        for base, datos in sorted(agrupados.items()):
            ns_txt = f"{datos.get('NS', {}).get('estado', 'SIN DATOS')} ({datos.get('NS', {}).get('sensor', '--')})"
            eo_txt = f"{datos.get('EO', {}).get('estado', 'SIN DATOS')} ({datos.get('EO', {}).get('sensor', '--')})"
    ...
    def _procesar_comando_consola(self, linea: str) -> bool:
        ...
        if cmd == "salir":
            ...
            print("    ola <INT_XX_DIR>     -> Activar OLA_VERDE en una direccion (ej: ola INT_C3_NS)")
            print("    normal <INT_XX_DIR>  -> Forzar NORMAL en una direccion (ej: normal INT_C3_EO)")
            ...
            if not (posicion.endswith("_NS") or posicion.endswith("_EO")):
                print(f"[MONITOR] Error: Debes incluir la direccion (_NS o _EO) en la posicion (ej. {posicion}_NS)")
                return True
    ...
    def run(self) -> None:
        ...
        print()
        print("[MONITOR] Monitor activo. Comandos: ola <INT_XX_DIR> | normal <INT_XX_DIR> | estado | ayuda | salir")
        print()
